...
function blankWorkProgram(){return {shift:defaultShiftForHour(),loadingRows:Array.from({length:4},()=>({company:'',tanker:'',done:false})),manualTasks:Array.from({length:3},()=>({text:'',done:false})),analysisResults:{},shiftLog:''}}
function normalizeWorkProgram(v){const b=blankWorkProgram(),x=v&&typeof v==='object'?v:{};return {shift:WORK_SHIFT_TIMES[x.shift]?x.shift:b.shift,loadingRows:(Array.isArray(x.loadingRows)?x.loadingRows:[]).map(r=>({company:upper(r?.company||''),tanker:upper(r?.tanker||''),done:!!r?.done})),manualTasks:(Array.isArray(x.manualTasks)?x.manualTasks:[]).map(r=>({text:String(r?.text||''),done:!!r?.done})),analysisResults:x.analysisResults&&typeof x.analysisResults==='object'?x.analysisResults:{},shiftLog:String(x.shiftLog||'')}}
...
function renderWorkProgram(){const dateInput=document.getElementById('workProgramDate');if(!dateInput)return;const program=ensureWorkProgram();dateInput.value=state.workProgramDate;const shift=document.getElementById('workProgramShift');shift.value=program.shift;document.getElementById('analysisShiftLabel').textContent=program.shift+' • '+WORK_SHIFT_TIMES[program.shift].join(' · ');
 const loading=document.getElementById('loadingTasks');loading.innerHTML='';if(!program.loadingRows.length)program.loadingRows.push({company:'',tanker:'',done:false});program.loadingRows.forEach((row,i)=>{const item=document.createElement('div');item.className='task-row loading-task'+(row.done?' task-done':'');const done=document.createElement('input');done.type='checkbox';done.checked=row.done;done.setAttribute('aria-label','Ολοκληρώθηκε');done.onchange=()=>{row.done=done.checked;renderWorkProgram();save()};const company=makeInput(row.company,v=>row.company=v,'work-company-input','clients',true,{addToClients:true});const tanker=makeTaskInput(row.tanker,'ΟΝΟΜΑ / ΑΡΙΘΜΟΣ ΒΥΤΙΟΥ',v=>row.tanker=v);const remove=document.createElement('button');remove.type='button';remove.className='task-remove';remove.textContent='✕';remove.onclick=()=>{program.loadingRows.splice(i,1);renderWorkProgram();save()};item.append(done,company.wrap,tanker,remove);loading.appendChild(item)});
...
function bind(){bindTabs();bindClipboardButtons();
 ...
 document.getElementById('addLoadingTask').onclick=()=>{ensureWorkProgram().loadingRows.push({company:'',tanker:'',done:false});renderWorkProgram();save()};
